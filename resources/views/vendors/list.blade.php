...
<div id="app"  class="max-w-full mx-auto sm:justify-between sm:items-center">
    ...
    <div class="flex items-center justify-between px-4 py-3 sm:p-0  bg-gray-800 sm:hidden ">

        <div class="block lg:hidden">
            <button class="sidebar-sign block text-blue-500 hover:text-white focus:text-white focus:outline-none" type="button" onclick="openDropdown(event,'businesstype2')">
               ...
               <div class="hidden bg-gray-600 text-base z-50 float-left py-2 list-none text-left rounded shadow-lg mb-1" style="min-width:12rem" id="businesstype2">
                @foreach ($businessData as $item)
                <a href="/vendors/type/{{ $item->id }}" class="text-sm py-2 px-4 font-normal block w-full whitespace-nowrap  text-gray-100 hover:bg-white hover:text-black">
                   {{ $item->name }}
                </a>
                @endforeach
               </div>
