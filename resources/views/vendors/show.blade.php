    ...
    <div class="max-w-full mx-auto">
        <div class="flex ">
            ...
            <div class="w-full">
                ...
                <div class="w-full lg:max-w-full lg:flex mx-auto  p-2 m-2 rounded-lg">
                    @if ($vendor->imagefile == '')
                        <div class="lg:w-1/2  items-center justify-center bg-white">
                            <img class="h-auto p-4 rounded-md" src="/images/store.jpg" alt="{{ $vendor->name }}">
                    ...
                    @else
                        <div class="lg:w-1/2  items-center justify-center bg-white">
                            <img class="h-auto p-4 rounded-md" src="{{ Storage::url($vendor->imagefile) }}"
                                alt="{{ $vendor->name }}">
                        </div>
                    @endif
                    <div class="lg:w-1/2 bg-white rounded-b flex  justify-between leading-normal ">
                        ...
                        <div class="mb-4 p-2">
                            ...
                            <div class="">
                                <a href="/vendors/type/{{ $vendor->businesstype_id }}"
                                    class="text-blue-500 hover:text-blue-700">
                                    ประเภทธุรกิจ: {{ $vendor->businesstype->name }}
                                </a>
                                <div class="items-center  text-gray-600 ">การดู : {{ $vendor->visits()->count() }} ครั้ง
                                </div>

                            </div>
                            <p
                                class="w-full bg-gray-100 py-2 mb-2 px-2 text-base text-gray-800 flex items-center rounded-lg mt-2">
                            ...
                            <div class="fb-share-button" data-href="{{ $open_graph['url'] }}" data-layout="box_count">
                            </div>
                ...
                <div class="flex lg:flex-row flex-col  text-base font-semibold lg:justify-center text-center ">
                    ...
                    @if (!empty($vendor->phoneno))
                        <a class="w-48 mx-auto m-1 text-center items-center rounded-full border border-blue-500 p-2 lg:mx-2 text-blue-500 hover:text-gray-100 hover:bg-blue-500"
                            href="tel:{{ $vendor->phoneno }}">
                            <svg xmlns="http://www.w3.org/2000/svg" class="fill-current absolute mr-2" viewBox="0 0 24 24"
                                width="24" height="24">
                    ...
                    @if (!empty($vendor->weburl))
                        <a class="w-48 mx-auto m-1 items-center rounded-full border border-blue-500 p-2 lg:mx-2 text-blue-500 hover:text-gray-100 hover:bg-blue-500"
                            href="{{ $vendor->weburl }}" target="_blank">
                            <svg xmlns="http://www.w3.org/2000/svg" class="fill-current absolute mr-2" viewBox="0 0 24 24"
                                width="24" height="24">
                    ...
                    @if (!empty($vendor->email))
                        <a class="w-48  mx-auto m-1 items-center rounded-full border border-blue-500 p-2 lg:mx-2 text-blue-500 hover:text-gray-100 hover:bg-blue-500"
                            href="mailto:{{ $vendor->email }}" target="_blank">
                            <svg xmlns="http://www.w3.org/2000/svg" class="fill-current absolute mr-2" viewBox="0 0 24 24"
                                width="24" height="24">
                                <path class="heroicon-ui"
                    ...
                    @if (!empty($vendor->facebook))
                        <a class="w-48 mx-auto m-1 items-center rounded-full border border-blue-500 p-2 lg:mx-2 text-blue-500 hover:text-gray-100 hover:bg-blue-500"
                            href="{{ $vendor->facebook }}" target="_blank">
                            <svg xmlns="http://www.w3.org/2000/svg" class="fill-current absolute mr-2" viewBox="0 0 24 24"
                                width="24" height="24">
                                <path
                                    d="M17,1H3C1.9,1,1,1.9,1,3v14c0,1.101,0.9,2,2,2h7v-7H8V9.525h2V7.475c0-2.164,1.212-3.684,3.766-3.684l1.803,0.002v2.605 h-1.197C13.378,6.398,13,7.144,13,7.836v1.69h2.568L15,12h-2v7h4c1.1,0,2-0.899,2-2V3C19,1.9,18.1,1,17,1z" />
                            </svg>
                    ...
                    @if (!empty($vendor->line))
                        @if (substr($vendor->line, 0, 1) == '@')
                            <a class="w-48 mx-auto m-1 items-center rounded-full border border-blue-500 p-2 lg:mx-2 text-blue-500 hover:text-gray-100 hover:bg-blue-500"
                                href="http://line.me/R/ti/p/{{ $vendor->line }}" target="_blank">
                                <svg xmlns="http://www.w3.org/2000/svg" class="fill-current absolute mr-2"
                        ...
                        @else
                            <a class="w-48 mx-auto m-1 items-center rounded-full border border-blue-500 p-2 lg:mx-2 text-blue-500 hover:text-gray-100 hover:bg-blue-500"
                                href="http://line.me/ti/p/~{{ $vendor->line }}" target="_blank">
                                <svg xmlns="http://www.w3.org/2000/svg" class="fill-current absolute mr-2"
                                    viewBox="0 0 24 24" width="24" height="24">
                                    <path
                                        d="M10 15l-4 4v-4H2a2 2 0 0 1-2-2V3c0-1.1.9-2 2-2h16a2 2 0 0 1 2 2v10a2 2 0 0 1-2 2h-8zM5 7v2h2V7H5zm4 0v2h2V7H9zm4 0v2h2V7h-2z" />
                ...
                <div class="lg:flex lg:justify-between  p-4 lg:bg-gray-300">
                    <div class="flex flex-col items-center lg:w-1/2 w-full lg:flex lg:flex-row mx-auto">
                        <img class=" lg:w-14 lg:h-14  w-16 h-16 rounded-full lg:mr-2"
                            src="{{ Storage::url($vendor->user->avatar) }}" alt="{{ $vendor->user->name }}">
                        <div class="text-sm lg:text-left text-center">
                            <p class="text-gray-900 leading-none">สมาชิกผู้โพส : {{ $vendor->user->name }}</p>
                            <p class="text-gray-900 leading-normal">เข้าร่วมเมื่อ :
                                {{ formatDateThai($vendor->user->created_at) }}</p>
                            <p class="text-gray-600 mb-2">จำนวนผู้ติดตาม :
                                {{ $vendor->user->followers()->get()->count() }} </p>
