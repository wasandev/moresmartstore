<div class="grid grid-cols-1  md:grid-cols-3 ">
    @foreach ($vendors as $vendor)
        <div>
            <a href="/vendors/{{ $vendor->id}}" class="flex flex-col flex-1 shadow  hover:shadow-xl translateY-2px m-2  no-underline transition">
                @if ($showimage)
                    <div class="aspect-16x9"
                     style="background:url('{{  Storage::url($vendor->imagefile) }}') no-repeat center center/cover">
                    </div>
                @endif
                <p class="text-base font-semibold text-center text-blue-700  ">{{ $vendor->name }} </p>
                ...
                <div class="px-2 bg-white subpixel-antialiased">
                    ...
                    <div class="w-full flex flex-row border-t border-gray-100 text-sm">
