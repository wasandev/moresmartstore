<div class="grid grid-cols-1  md:grid-cols-2 ">
    @foreach ($posts as $post)
        <div>
        <a href="/post/{{ $post->id }}" class="flex flex-col flex-1 border border-gray-200 shadow hover:shadow-lg translateY-2px m-2 p-2 lg:m-4 lg:p-4 active:no-underline transition">
                @if ($showimage)
                ...
                <div class="p-2  text-left subpixel-antialiased">
                    ...
                    <div class="w-full flex flex-row border-t border-gray-100 text-sm font-thin">
                        ...
                        <div class="text-right w-2/3 mt-1">{{formatDateThai( $post->published_at )}}</div>
                    </div>
                </div>
            </a>
        </div>
