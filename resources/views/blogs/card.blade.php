    ...
    @foreach ($blogs as $blog)
        <div>
            <a href="/blogs/{{ $blog->slug }}" class="flex flex-col  border border-gray-200 hover:shadow-lg translateY-2px m-2 p-2 lg:m-4 lg:p-4 no-underline transition">
                ...
                <div class="px-2 flex flex-row w-full   border-t border-gray-100 text-sm font-thin ">

                    <div class="text-left w-1/3 m-2">
                    ...
                    <div class="text-right w-2/3 m-2">{{ $blog->blog_cat->name }}</div>
                </div>
            </a>
        </div>
    @endforeach
