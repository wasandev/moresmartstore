<div class="flex flex-col xl:mx-0 ">
    ...
    <a href="/blogs/{{ $blog->slug }}" class="bg-blue-400 flex flex-col flex-1 rounded-t-lg shadow hover:shadow-lg translateY-2px m-2 no-underline transition">
        @if ($showimage)
        <div class="aspect-16x9 rounded-t"
            style="background:url('{{  Storage::url($blog->blog_image) }}') no-repeat center center/cover">
        </div>
        @endif
        <h1 class="p-2 text-gray-100 text-base font-light text-center">{{ $blog->title }} </h1>
