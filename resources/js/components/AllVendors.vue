<template>
     <div  class="rounded-lg">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4 gap-2">
            <div class="w-full" v-for="(allvendor,i) in allvendors" :key=i>
                <div class="flex">
                    <div class="flex flex-col flex-1 bg-white rounded shadow hover:shadow-lg m-2 p-2 no-underline transition">
                        <div class="aspect-16x9 rounded" v-bind:style="{ 'background': 'url(' + allvendor.path +')' +'no-repeat center center/cover' } ">
                        </div>
                        <div class="p-2 flex flex-col flex-1 bg-gray-100  rounded-b-lg subpixel-antialiased">
                            <p class="mb-2 text-grey-900 text-sm font-semibold text-left">{{allvendor.name}}</p>
                        </div>
                         <div class="mb-3 w-full flex-1 mx-auto text-left text-sm font-thin ">
                                <p> {{ truncateText(allvendor.description) }} </p>
                            </div>
...
<script>
 import {mapGetters} from 'vuex'
    ...
    export default {
        ...
        mounted() {
                this.$store.dispatch('GET_ALLVENDORS')

                window.Echo.channel('searchvendor')
                    .listen('.searchVendorResults', (e) => {
                        this.$store.commit('SET_ALLVENDORS', e.vendors)
                    })
        ...
        methods: {
            ...
            viewVendor(VendorIndex) {
                const showvendor = this.allvendors[VendorIndex];
